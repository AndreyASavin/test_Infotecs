...
// Объявил основные элементы страницы в виде переменных
let tableBody = document.querySelector("#table_body"); //тело таблицы (ячейки со значениями элементов массива)
let columnArrows = document.querySelectorAll(".arrow"); // сортировочные стрелки в заголовках
let modalBack = document.querySelector('#modal_background'); // контейнер всего модального окна
let modalInput = document.querySelector("#modal_input"); // контейнер для инпутов (редактирование значений)
let modalButton = document.querySelector("#modal_button"); // контейнер для кнопки подтверждения редактирования
let pages = document.querySelector("#pages"); // контейнер для номеров страниц

/* Объявил новый пустой массив. Сделал для того, чтобы развернуть объект name:{firstName,lastName}, записать имя и фамилию
 как новые "не вложенные" свойства нового объекта и сохранить в этот массив. Позже по этим новым свойствам будет производиться
 сортировка используя data- атрибуты стрелок в заголовках таблицы */
let newArray = [];

/* Перебрал исходный массив, на основе его элементов создал новые и поместил в новый массив. Все работы теперь
будут вестись с массивом newArray */
for (let n=0; n<mainArray.length; n++){
    let newItem = {};
    newItem.id=mainArray[n].id;
    newItem.firstName=mainArray[n].name.firstName;
    newItem.lastName=mainArray[n].name.lastName;
    newItem.about=mainArray[n].about;
    newItem.eyeColor=mainArray[n].eyeColor;
    newArray.push(newItem);
};

// Функция отрисовки таблицы, в качестве параметра позже примет массив
const drawTable = (data) => {
    tableBody.innerHTML=''; // изначально очищаем контейнер тела таблицы для будущих перерисовок
    for (let i=0; i<data.length; i++) { // счетчик i - индекс каждого элемента(объекта) в массиве
        let row = document.createElement('tr'); // создаем строку и заполняем ее значениями свойств элемента с индексом i
        row.classList.add = 'oneRow';
        row.innerHTML = `
             <td data-td="${data[i].firstName}">${data[i].firstName}</td> 
             <td data-td="${data[i].lastName}">${data[i].lastName}</td>
             <td data-td="${data[i].about}"><div class="tablediv">${data[i].about}</div></td>
             <td data-td="${data[i].eyeColor}" style="background-color:${data[i].eyeColor}"></td>
        `;
        // назначил data- атрибуты ячейкам, потребуется для сортировки таблицы
        // чтобы цвет глаз был в качестве фона, вставил значение в background-color

        tableBody.appendChild(row); // вставил заполненную строку в тело таблицы

        row.onclick = function(){ // на каждую строку повесил слушатель события, при клике будет создаваться модальное окно
            modalBack.style.display='block'; // сделал видимым контейнер модального окна, он закрывает собой всю таблицу и не дает случайно нажать на нее
            modalInput.innerHTML=``; // изначально контейнеры для инпутов и кнопки очищены
            modalButton.innerHTML=``;
            columnArrows.forEach(function(item){ 
                // сделал стрелки сортировки неактивными, иначе сортировка при открытом модальном окне сломает логику отображения
                item.setAttribute('disabled','disabled');
            })

            let inputName = document.createElement('input'); // создал инпут для редактирования имени (свойство name массива)
            inputName.type="text";
            inputName.value=data[i].firstName; // по умолчанию уже заполнено значением name элемента, находящимся в row, на который кликнули
            inputName.oninput = function(){ // в зависимости от изменения (не изменения) значения value в инпуте, название кнопки будет меняться
                if (inputName.value!==data[i].firstName) { // если value не совпадает с текущим значением свойства, но будет "Изменить"
                    changeButton.innerText='Изменить'
                } else { // если совпадает (значит изменений пользователь в инпут не внес), то название кнопки будет "Закрыть"
                    changeButton.innerText='Закрыть'
                }
            }
            let inputLastName = document.createElement('input'); // по аналогии создал инпут для фамилии (lastName)
            inputLastName.type="text";
            inputLastName.value=data[i].lastName;
            inputLastName.oninput = function(){
                if (inputLastName.value!==data[i].lastName) {
                    changeButton.innerText='Изменить'
                } else {
                    changeButton.innerText='Закрыть'
                }
            }
            let inputAbout = document.createElement('input'); // по аналогии создал инпут для описания (about)
            inputAbout.type="text";
            inputAbout.value=data[i].about;
            inputAbout.oninput = function(){
                if (inputAbout.value!==data[i].about) {
                    changeButton.innerText='Изменить'
                } else {
                    changeButton.innerText='Закрыть'
                }
            }
            let inputEyeColor = document.createElement('input'); // по аналогии создал инпут для цвета глаз (eyeColor)
            inputEyeColor.type="text";
            inputEyeColor.value=data[i].eyeColor;
            inputEyeColor.oninput = function(){
                if (inputEyeColor.value!==data[i].eyeColor) {
                    changeButton.innerText='Изменить'
                } else {
                    changeButton.innerText='Закрыть'
                }
            ...
            // все созданные инпуты вставил в контейнер для инпутов modalInput
            modalInput.append(inputName,inputLastName,inputAbout,inputEyeColor);

            // создал кнопку для подтверждения редактирования
            let changeButton = document.createElement('button');
            changeButton.style.cursor='pointer';
            changeButton.innerText='Закрыть'; // кнопка назвается "Закрыть" по умолчанию, изменения названия описал выше

            /* повесил событие на кнопку. По нажатию на кнопку элемент с индеком i, на котором было вызвано событие, 
            примет новые значения свойств из инпутов модального окна */
            changeButton.onclick = function(){  
                data[i].firstName = inputName.value;
                data[i].lastName = inputLastName.value;
                data[i].about = inputAbout.value;
                data[i].eyeColor = inputEyeColor.value;
                drawTable(data); // после изменения в элементе массива вызвал функцию отрисовки таблицы заново, чтобы изменения отобразились в таблице
                modalInput.innerHTML = ``; // очистил контейнер инпутов и кнопки, чтобы скрыть модальное окно
                modalButton.innerHTML = ``;
                modalBack.style.display='none'; // снова сделал невидимым контейнер модального окна
                columnArrows.forEach(function(item){
                    // снова сделал стрелки активными
                    item.removeAttribute('disabled')
                })
            }            
            modalButton.appendChild(changeButton); // вставил саму кнопку в соответствующий контейнер
        }
    } 
};

// Реализация пагинации
let currentPage = 1; // текущая страница, по умолчанию первая
let displayedItems = 10; // количество элементов, отображаемых в таблице (количество строк в таблице)
let maxPage = Math.ceil(newArray.length/displayedItems) // посчитал максимальное число страниц, согласно количеству элементов в таблице, 
// приведен к округлению вверх. В случае нецелочисленного деления кол-ва элементов на 10, остаток от деления будет отрисован на последней странице

/* отображаемые на странице элементы - это ссылка на часть исходного массива. 
методом slice получаем часть исходного массива. 
условия - начальный элемент как (номер страницы - 1 * кол-во элементов) и конечный элемент как (начальный элемент + кол-во элементов) */
let displayedArray = newArray.slice((currentPage-1)*displayedItems, (currentPage-1)*displayedItems+displayedItems);

//вызвал функцию отрисовки таблицы и передали в нее выделенную часть массива
drawTable(displayedArray);

for (let num = 1; num<=maxPage; num++){ // счетчик num - это номер страницы
    let pageElem = document.createElement('div'); // создал элемент для номера страницы и передал в него номер
    pageElem.innerHTML = `<div>${num}</div>`;
    pageElem.classList.add('page_elem');
    if (currentPage == num) { 
        pageElem.classList.add('active') // если текущая страница равна номеру страницы в элементе - выделение классом active
    }
    pages.appendChild(pageElem); // вставил страницу в контейнерный элемент для страниц

    // функция перехода между страницами
    pageElem.onclick = function(){
        currentPage = num; // при нажатии текущей странице передается значение страницы в элементе, на который кликнули
        let activePage = document.querySelector('#pages div.active'); // нашел текущий элемент с классом active
        activePage.classList.remove('active'); // убрал клас 
        pageElem.classList.add('active'); // повесил класс на наш элемент (на который кликнули)

        // заново определил отображаемые элементы таблицы на странице (поскольку было изменение текущей страницы - ее нужно заново передать)
        displayedArray = newArray.slice((currentPage-1)*displayedItems, (currentPage-1)*displayedItems+displayedItems);
        drawTable(displayedArray); // вызвал функцию отрисовки таблицы
        columnArrows.forEach(function(item){ // удалил классы и стерли data- атрибуты у стрелок сортировки,
        // чтобы при переходе между страницами выделение стрелок удалялось и сортировка была по умолчанию
            item.dataset.sort='unsorted';
            item.classList.remove('up', 'down');
        });
        tableHeaders.forEach((header)=>{
            header.classList.remove('hidden') // сбросил скрытие заголовков таблицы
        });
        hideDivs.forEach((oneDiv)=>{
            oneDiv.innerText=`Скрыть ${oneDiv.dataset.name}`; // сбросил скрытие ячеек (столбцов)
            oneDiv.dataset.show='show'
        })
    }
}

// создал изменение поведения сортировочных стрелок и сортировку элементов в таблице
/* В HTML-файле заранее отрисовал заголовки таблицы и добавил в них button-ы стрелок с data- атрибутами th. Значения
этих атрибутов совпадают с data- атрибутами td в ячейках отрисовываемой таблицы. Соответственно, data- атрибут в стрелке 
заголовка "Имя" совпадает с data- атрибутом ячеек, содержащих свойство name объектов, и так далее для всех отображаемых
значений свойств в таблице. Также добавил стрелкам дополнительный data- атрибут sort, по которому буду отслеживать,
используется ли сортировка по данному параметру, или нет. По умолчанию - unsorted, так как сортировки в начале никакой нет */
for (let arrow of columnArrows) {
    arrow.addEventListener('click', function(){
        let dataValue = arrow.dataset.th; // создал переменную, в которую при клике будет помещаться значение data- атрибута стрелки, на которую кликнули
        // соответственно сортировка будет опираться на data- атрибут th

        // заложил 3 возможных варианта условий клика на стрелку сотрировки
        // 1 вариант - если на данной стрелке не было клика - реализую сортировку "от А до Я"
        if (arrow.dataset.sort == 'unsorted') { // если data- атрибут unsorted
            arrow.dataset.sort = 'sorted'; // при клике изменяю атрибут на sorted
            arrow.classList.add('down'); // добавляю класс down (соответствует сортировке "от А до Я")
            displayedArray = displayedArray.sort((a,b)=>a[dataValue].localeCompare(b[dataValue])); // реализую сортировку методом сравнения строк localeCompare с соответствующим параметром сотрировки 
            drawTable(displayedArray); // вызвал функцию отрисовки таблицы по отсортированному массиву
            columnArrows.forEach(function(item){ // удалил классы и изменяю data- атрибут sort у других стрелок сортировки
                if (item.dataset.th !== dataValue){ // параметр определения у каких стрелок нужно все обнулить - если data-th не равен переменной dataValue
                    item.classList.remove('up','down');
                    item.dataset.sort='unsorted';
                }
            })

        // 2 вариант - если на данной стрелке был клик и идет сортировка "от А до Я" - переход в сотрировку "от Я до А"
        } else if (arrow.dataset.sort == 'sorted' && arrow.classList.contains('down')) {
            arrow.classList.remove('down'); // удалил класс down
            arrow.classList.add('up'); // добавил класс up (соответствует сотрировке "от Я до А")
            displayedArray = displayedArray.sort((a,b)=>b[dataValue].localeCompare(a[dataValue])); // реализовал сортировку с измененным порядком
            drawTable(displayedArray); // вызвал функцию отрисовки таблицы по отсортированному массиву

        // 3 вариант - если на данной стрелке был клик и идет сортировка "от Я до А" - снятие сортировки
        } else if (arrow.dataset.sort == 'sorted' && arrow.classList.contains('up')) {
            arrow.classList.remove('up'); // удалил класс up
            arrow.dataset.sort = 'unsorted' // data-sort сделал unsorted
            // заново определил отображаемый массив как копию части исходного несортированного массива
            displayedArray = newArray.slice((currentPage-1)*displayedItems, (currentPage-1)*displayedItems+displayedItems);
            drawTable(displayedArray); // отрисовка таблицы
        }
    })
};

// Скрытие и показ колонок
let hideDivs = document.querySelectorAll('.hide'); //кнопки скрытия
let tableHeaders = document.querySelectorAll('th'); //заголовки таблицы

hideDivs.forEach(function(oneDiv){
    oneDiv.innerText=`Скрыть ${oneDiv.dataset.name}`; // название кнопки по умолчанию
    oneDiv.onclick = function(){
        tableHeaders.forEach((header)=>{ // при нажатии на кнопку перебрал заголовки таблиц
            if (oneDiv.dataset.show=='show' && oneDiv.dataset.div==header.dataset.header){ // при совпадении data- атрибута, если колонка показана, скрываю
                oneDiv.dataset.show='hide';
                header.classList.add('hidden');
                header.dataset.hide='hide';
                oneDiv.innerText=`Показать ${oneDiv.dataset.name}`;
                document.querySelectorAll('td').forEach((elem)=>{// для скрытия ячеек пользуюсь индексом заголовка и ячейки в каждом ряду
                    if (elem.cellIndex==header.cellIndex){
                        elem.classList.add('hidden') // скрываю ячейку при совпадении индекса с индексом заголовка
                    }
                }) // аналогичные действия для того, чтобы отобразить скрытые ячейки
            } else if (oneDiv.dataset.show=='hide' && oneDiv.dataset.div==header.dataset.header){
                oneDiv.dataset.show='show'; 
                header.classList.remove('hidden');
                header.dataset.hide='show';
                oneDiv.innerText=`Скрыть ${oneDiv.dataset.name}`;
...
/*  КОММЕНТАРИИ:
    1) При переключении страниц сортировка обнуляется
    2) При переключении страниц сбрасывается скрытие столбцов
*/